...
describe('Test User', () => {
  ...
  const users = [
    new User('aman', 'Ayam Man', 'ayam', true),
    new User('bman', 'Burung Man', 'burung', false),
    new User('cman', 'Cicak Man', 'cicak'),
  ];

  describe(`"POST:${addUsersPath}, create user in db"`, () => {
  ...
  describe(`"GET:${getUsersPath}"`, () => {
    ...
    request was successful.`, (done) => {
      ...
      UserModel.create(new User("aaaa", "bbbb", "cccc"));
      UserModel.deleteOne({name: "bbbb"});
      //add more users
      UserModel.create(users[1]);
      UserModel.create(users[2]);
      ...
      .end((err: Error, res: Response) => {
        ...
        // Cast instance-objects to 'User' objects
        const retUsers = res.body.users.map((user: IUser) => {
          return new User(user.uname, user.name, user.pwdHash, user.admin);
        });
        expect(retUsers).toContain(users[0]);
        expect(retUsers).toContain(users[1]);
        expect(retUsers).toContain(users[2]);
        expect(res.body.error).toBeUndefined();
        done();
      });
    });
    ...
    describe(`"PUT:${updateUserPath}"`, () => {
      ...
      const userData = {
        user: new User('ayam', 'Ayam Man', 'ayam', true),
      };
